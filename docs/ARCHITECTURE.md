...
The browser loads static React assets and safe configuration from the same Worker. The browser holds source text, the original extraction, the edited card, follow-up wording and acknowledgment in memory. Reset/reload clears this content. Six reviewed fictional fixtures are bundled, and are available only for their exact source strings.
